...
// - For example, on Mac OS X running Adobe Illustrator CC 2014, the path would be: // Adobe Illustrator CC 2014/Presets/en_US/Scripts/ai2html.js
...
function main() {
	...
	var defaultSettings = {
		...
		image_format: 'png24', // Options: auto, png, png24, jpg, svg
		...
		image_output_path: '../../../../../static/images/',
	...
	function render(settings, customBlocks) {
		...
		if (isTrue(settings.create_config_file)) {
			...
			var yamlPath = docPath + (settings.config_file_path || 'config.json'),
				yamlStr = generateConfigJsonContent(settings);
	...
	function cleanObjectName(name) {
		return makeKeyword(name.replace('#', '').replace(/^(.+):.*$/, '$1'));
	}
	...
	function getDocumentName(customName) {
		var name =
			customName || docName || doc.name.replace(/(.+)\.[aieps]+$/, '$1').replace(/ +/g, '-');
	...
	function convertAiColor(color, opacity) {
		...
		if (color.typename == 'RGBColor') {
			...
			r = g = b = 0;
			o.warning =
				'The text "%s" has ' + color.typename + ' fill. Please fill it with an RGB color.';
		}
	...
	function convertAiTextStyle(aiStyle) {
		...
		if ('opacity' in aiStyle) {
			// cssStyle.filter =
			// 	'alpha(opacity=' + Math.round(aiStyle.opacity) + ')';
			// cssStyle['-ms-filter'] =
	...
	function getImageFolder(settings) {
		return pathJoin(docPath, settings.image_output_path, docName);
	}
	...
	function generateImageHtml(imgFile, imgId, imgClass, imgStyle, ab, settings) {
		function getImgDir() {
			return pathJoin(settings.image_source_path, docName);
		}
	...
	function generateConfigJsonContent(settings) {
		var meta = {};
	...
	function getResizerScript(containerId) {
		...
		var resizerJs =
			...
			');';
		return (
			"<script>\r\timport { onMount } from 'svelte';\r\tonMount(() => {\r\t\t" +
			resizerJs +
			'\r\t\t});\r</script>\r'
		);
	}
	...
	function generateOutputHtml(content, pageName, settings) {
		...
		// HTML
		html = '<div id="' + containerId + '" class="ai2html">';
		if (linkSrc) {
		...
		// CSS
		css = '<style>\r' + generatePageCss(containerId, settings) + content.css + '\r</style>\r';
		...
		htmlFileDestinationFolder = docPath + '/';
